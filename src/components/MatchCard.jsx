...
function MatchCard({ match, players, onUpdate }) {
  const { matches, updateAppState } = useAppData();
  ...
  const handlePenaltyStatusChange = async (newStatus) => {
    ...
    try {
      const updatedMatches = matches.map((m) =>
        m.id === match.id ? { ...m, penaltyPaid: newStatus } : m
      );

      await updateAppState({ matches: updatedMatches });
